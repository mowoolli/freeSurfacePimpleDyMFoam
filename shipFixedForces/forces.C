...
namespace Foam
{
    defineTypeNameAndDebug(shipFixedForces, 0);
}


// * * * * * * * * * * * * Protected Member Functions  * * * * * * * * * * * //

Foam::wordList Foam::shipFixedForces::createFileNames(const dictionary& dict) const
{
...
void Foam::shipFixedForces::writeFileHeader(const label i)
{
    if (i == 0)
    {
        // force data
...
void Foam::shipFixedForces::initialise()
{
...
Foam::tmp<Foam::volSymmTensorField> Foam::shipFixedForces::devRhoReff() const
{
...
Foam::tmp<Foam::volScalarField> Foam::shipFixedForces::mu() const
{
...
Foam::tmp<Foam::volScalarField> Foam::shipFixedForces::rho() const
{
    if (rhoName_ == "rhoInf")
    {
...
Foam::scalar Foam::shipFixedForces::rho(const volScalarField& p) const
{
...
void Foam::shipFixedForces::applyBins
(
...
void Foam::shipFixedForces::writeForces()
{
    Info(log_)
...
void Foam::shipFixedForces::writeBins()
{
    if (nBin_ == 1)
    {
        return;
...
Foam::shipFixedForces::shipFixedForces
(
    const word& name,
...
void Foam::shipFixedForces::read(const dictionary& dict)
{
    if (active_)
    {
        initialised_ = false;
...
void Foam::shipFixedForces::execute()
{
    // Do nothing - only valid on write
}


void Foam::shipFixedForces::end()
{
    // Do nothing - only valid on write
}


void Foam::shipFixedForces::timeSet()
{
    // Do nothing - only valid on write
}


void Foam::shipFixedForces::write()
{
    calcForcesMoment();
...
void Foam::shipFixedForces::calcForcesMoment()
{
    initialise();
...
//            Info << "mesh.C().boundaryField()[patchI] = " << mesh.C().boundaryField()[patchI] << endl;
//            Info << "coordSys_.origin()" << coordSys_.origin() << endl;
...
Foam::vector Foam::shipFixedForces::forceEff() const
{
    return sum(force_[0]) + sum(force_[1]) + sum(force_[2]);
}


Foam::vector Foam::shipFixedForces::momentEff() const
{
